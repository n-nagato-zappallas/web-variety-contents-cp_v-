...
#curl -Ss 'http://172.24.240.229/cp_v/index.php' --data "${QUERY_STRING}" -XPOST -o index_${sur}.html.tmp
...
### input part
t="input"
for file in `find /srv/www/php/htdocs/cp_v/mynumber/data/*.php -type f`
do
	file_name=`basename ${file}`
	menu_id=`echo ${file_name} | sed -e 's/.php//g'`

	if [ `echo ${menu_id} | grep 'no'` ]; then
        if [ ${menu_id} != 'no_002' ] && [ ${menu_id} != 'no_00X' ]; then
			QUERY_STRING="event=${event}&isp=${isp}&t=${t}&menu_id=${menu_id}"
			curl -Ss 'http://172.24.240.229/cp_v/index.php' --data "${QUERY_STRING}" -XPOST -o ${menu_id}.html.tmp
